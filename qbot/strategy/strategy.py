...
class Strategy(ABC):
    ...
    def eval_perf(self, start_time = None, end_time = None, EVAL_PERIOD=1):
        perf_dict = {}
        perf_str = ""

        
        if start_time is None:
            start_time = self._compute_df['timestamp'].iloc[0]
        if end_time is None:
            end_time = self._compute_df['timestamp'].iloc[-1]
        compute_df = self._compute_df[(self._compute_df['timestamp'] >= start_time) & (self._compute_df['timestamp'] <= end_time)]
        ...
        perf_dict["duration"] = len(compute_df) / YEAR_DIV_NUM

        # Print strategy statistics
        total_return = compute_df["asset_value"].iloc[-1] / compute_df["asset_value"].iloc[0] - 1
        annual_return = (1.0 + total_return) ** (YEAR_DIV_NUM / len(compute_df)) - 1
        perf_dict["total_return"] = total_return
        perf_dict["annual_return"] = annual_return

        reference_return = (
            compute_df[self.reference_index].iloc[-1] / compute_df[self.reference_index].iloc[0] - 1
        )
        annual_reference_return = (1.0 + reference_return) ** (YEAR_DIV_NUM / len(compute_df)) - 1
        perf_dict["reference_return"] = reference_return
        perf_dict["annual_reference_return"] = annual_reference_return

        mdd = (compute_df["asset_value"].div(compute_df["asset_value"].cummax()) - 1).min()
        perf_dict["mdd"] = mdd

        # Calculate alpha, beta, and gamma
        alpha, beta, gamma = calculate_alpha_beta_gamma(
            compute_df[self.reference_index],
            compute_df["asset_value"],
            periods=EVAL_PERIOD,
        )
        ...
        asset_return = compute_df["asset_value"].apply(np.log).diff(periods=EVAL_PERIOD)
        ...
        ref_return = compute_df[self.reference_index].apply(np.log).diff(periods=EVAL_PERIOD)
    ...
    def plot_perf(self, start_time = None, end_time = None):
        ...
        weight_cols = self.get_weight_cols()
        cutoff_num = 0
        while cutoff_num < len(compute_df):
            # Check if all weights are set
            if all(compute_df[col].iloc[cutoff_num] is not None for col in weight_cols):
                break
            cutoff_num += 1
        timestamp_cutoff = compute_df["timestamp"].iloc[cutoff_num:]
        ...
        for col in compute_df.columns:
            if col == "timestamp" or col == "asset_value" or col.endswith("_weight") or col.endswith("_close"):
                continue
            # normalize column
            cutoff_series = compute_df[col].iloc[cutoff_num:]
        ...
        cutoff_series = compute_df["asset_value"].iloc[cutoff_num:]
        max_val = cutoff_series.max()
        min_val = cutoff_series.min()
        ...
        cutoff_series = compute_df[self.reference_index].iloc[cutoff_num:]
        max_val = cutoff_series.max()
        min_val = cutoff_series.min()
        ...
        for col in weight_cols:
            ...
            ax2.plot(
                timestamp_cutoff,
                compute_df[col].iloc[cutoff_num:] / compute_df[col].iloc[cutoff_num],
                label=col,
                alpha=0.5,
                linewidth=1,
            )
